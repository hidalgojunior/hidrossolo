...
curl <?= e($app_url) ?>/api/status

# Dashboard (autenticado - com Bearer token)
curl -H "Authorization: Bearer hidrossolo_api_2026_token_admin" \
     <?= e($app_url) ?>/api/dashboard

# Listar serviços
curl -H "Authorization: Bearer hidrossolo_api_2026_token_admin" \
     <?= e($app_url) ?>/api/servicos

# Contatos não lidos
curl -H "Authorization: Bearer hidrossolo_api_2026_token_admin" \
     <?= e($app_url) ?>/api/contatos?status=new

# Enviar contato (público - sem token)
curl -X POST <?= e($app_url) ?>/api/contato \
...
<?php foreach ($schemas as $table => $columns) { ?>
                    <div class="accordion-item">
                        <h2 class="accordion-header">
                            <button class="accordion-button collapsed py-2" type="button" data-bs-toggle="collapse" data-bs-target="#sch-<?= e($table) ?>" style="font-size:0.85rem">
                                <code class="me-2"><?= e($table) ?></code>
                                <span class="badge bg-secondary ms-2"><?= e(count($columns)) ?> cols</span>
                            </button>
                        </h2>
