...
<form method="POST" action="/admin/cms/lgpd">
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <select name="maintenance_mode" class="form-select">
                        <option value="1" <?= e(($settings['maintenance_mode'] ?? '0') == '1' ? 'selected' : '') ?>>🔴 Ativado</option>
                        <option value="0" <?= e(($settings['maintenance_mode'] ?? '0') == '0' ? 'selected' : '') ?>>🟢 Desativado</option>
                    </select>
                ...
                <div class="col-md-8">
                    ...
                    <input type="text" name="maintenance_message" class="form-control" value="<?= e($settings['maintenance_message'] ?? 'Estamos realizando melhorias no site. Voltaremos em breve!') ?>">
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <select name="cookie_consent_enabled" class="form-select">
                        <option value="1" <?= e(($settings['cookie_consent_enabled'] ?? '1') == '1' ? 'selected' : '') ?>>Ativado</option>
                        <option value="0" <?= e(($settings['cookie_consent_enabled'] ?? '') == '0' ? 'selected' : '') ?>>Desativado</option>
                    </select>
                </div>
                <div class="col-md-4">
                    <label class="form-label">E-mail DPO (Encarregado LGPD)</label>
                    <input type="email" name="lgpd_contact_email" class="form-control" value="<?= e($settings['lgpd_contact_email'] ?? $config['email'] ?? '') ?>">
                </div>
                <div class="col-12">
                    <label class="form-label">Texto do Banner de Cookies</label>
                    <input type="text" name="cookie_consent_text" class="form-control" value="<?= e($settings['cookie_consent_text'] ?? 'Este site utiliza cookies para melhorar sua experiência. Ao continuar navegando, você concorda com nossa Política de Privacidade.') ?>">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <textarea name="content_politica-privacidade" class="form-control editor" rows="10"><?= e($pages['politica-privacidade']['content'] ?? '<h4>Política de Privacidade</h4>
...
<p>Você pode solicitar a exclusão ou alteração dos seus dados a qualquer momento pelo e-mail de contato.</p>') ?></textarea>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <textarea name="content_politica-cookies" class="form-control editor" rows="8"><?= e($pages['politica-cookies']['content'] ?? '<h4>Política de Cookies</h4>
...
<p>Você pode desabilitar cookies nas configurações do seu navegador.</p>') ?></textarea>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <textarea name="content_termos-uso" class="form-control editor" rows="8"><?= e($pages['termos-uso']['content'] ?? '<h4>Termos de Uso</h4>
