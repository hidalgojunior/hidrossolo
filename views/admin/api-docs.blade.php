        ...
        <div class="card mb-4">
            <div class="card-body">
                ...
                <table class="table table-sm mt-3">
                    <tr><td style="width:160px"><strong>Base URL</strong></td><td><code>{{ $app_url }}/api</code></td></tr>
...
# Listar serviços
curl -H "Authorization: Bearer hidrossolo_api_2026_token_admin" \
     {{ $app_url }}/api/servicos

# Contatos não lidos
curl -H "Authorization: Bearer hidrossolo_api_2026_token_admin" \
     {{ $app_url }}/api/contatos?status=new

# Enviar contato (público - sem token)
curl -X POST {{ $app_url }}/api/contato \
...
@foreach ($schemas as $table => $columns)
                    <div class="accordion-item">
                        <h2 class="accordion-header">
                            <button class="accordion-button collapsed py-2" type="button" data-bs-toggle="collapse" data-bs-target="#sch-{{ $table }}" style="font-size:0.85rem">
                                <code class="me-2">{{ $table }}</code>
                                <span class="badge bg-secondary ms-2">{{ count($columns) }} cols</span>
                            </button>
                        </h2>
                        <div id="sch-{{ $table }}" class="accordion-collapse collapse" data-bs-parent="#tableSchemas">
                            <div class="accordion-body p-2" style="font-size:0.75rem">
                                @foreach ($columns as $col)
                                <div class="py-1 border-bottom {{ $loop->last ? 'border-bottom-0' : '' }}">
                                    <code class="text-primary">{{ explode(' ', $col)[0] }}</code>
                                    <span class="text-muted ms-1">{{ preg_replace('/^[^\s]+\s/', '', $col) }}</span>
                                </div>
                                @endforeach
                            </div>
                        </div>
